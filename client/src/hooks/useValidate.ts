...
type TValidationTypes = string | "" | "required" | "number" | "string" | "email" | "confirm_password";
...
const useValidate: IUseValidate = (value, old = null) => {
    const [state, setState] = useState<ValidatorState>(prevStateConverter(value, old));
    ...
    const validOnChange: TValidOnChange = ({ value, name }, callback = () => void 0) => {
